...
`openspec_plan` is `coding-harness`'s deterministic, OpenSpec-CLI-driven planning sub-workflow for `code_parallel`. It replaces the retired `design_docs.json` sub-workflow and `code_parallel`'s old freeform LLM decomposition step: sequencing is driven by `openspec status`'s dependency graph rather than agent judgment, and review reuses `design_docs.json`'s existing human-or-AI-judge loop unchanged in mechanism.
