...
2. A bounded outer loop (`DO_WHILE`, capped by `openspecMaxIterations`, same mechanism as today's `designMaxIterations`), each iteration:
   a. Calls **`openspec_artifact_drain`, a separate `SUB_WORKFLOW`** — not an inline nested `DO_WHILE`. Conductor does not support a `DO_WHILE` nested directly inside another `DO_WHILE`'s `loopOver` (confirmed both against the product docs and live: it silently stops advancing after the first fan-out with no error at all, on both SQLite- and Postgres-backed servers — see the Risks section). `openspec_artifact_drain.json` owns its own inner `DO_WHILE` that repeatedly calls `openspec_status`, JQ-selects not-yet-generated-this-pass artifacts with `status != "blocked"`, calls `openspec_instructions` per artifact, and fans out a `coding_agent`-backed `openspec_generate_artifact` sub-workflow for each, until nothing new is selectable — draining the whole dependency graph in one outer pass.
...
Verified live end-to-end (real Conductor server, real worker fleet, real Claude API) against both a Postgres-backed and a SQLite-backed server: proposal → design+specs (parallel fan-out) → tasks.md → AI-judge approval → `openspec_tasks_to_subtasks` → `code_parallel`'s coding fan-out → clean merge, no conflicts, correct code. Zero SQLite errors on the SQLite run — confirming the drain-loop-as-sub-workflow fix (not the database backend) was the actual blocker.
...
- **[Risk, realized] A `DO_WHILE` nested directly inside another `DO_WHILE`'s `loopOver` is not supported by Conductor.** The original design inlined the artifact-drain loop as a nested `DO_WHILE` inside `openspec_loop`. Live testing showed it silently stops advancing after the first parallel fan-out (e.g. `design`+`specs` completing together) — no exception, no log line, the decider just never schedules the next iteration despite the loop condition correctly evaluating `true`. Reproduced identically on both SQLite- and Postgres-backed servers, ruling out a database-concurrency cause. → Mitigation (applied): extracted the inner loop into its own `openspec_artifact_drain.json` sub-workflow, invoked via `SUB_WORKFLOW` from `openspec_loop`'s `loopOver` (matching the product docs' own guidance: "To achieve a similar functionality as a nested loop, you can use a Sub Workflow task inside the Do While task"). Verified live end-to-end after the fix.
...
2. Add `openspec_plan.json` and `openspec_artifact_drain.json` sub-workflows (the latter holding the artifact-drain `DO_WHILE`, invoked via `SUB_WORKFLOW` — never nest a `DO_WHILE` directly inside another `DO_WHILE`).
