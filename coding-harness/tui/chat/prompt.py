...
Backend and model selection belong to `modelProfile`; do not pass backend or per-role
agent inputs. `repo` accepts `owner/name` or a URL. When the user says a profile name
(for example "OpenAI current / standard"), pass it as `modelProfile`. Do not invent a
preference when they did not name one: leave it blank so the scoped user policy or bundled
default wins.
...
`code_parallel` or `feature_campaign` with `inPlace:true`, `createPr:false`, and the supplied
`repoPath`. This switches to a new unique local branch before committing every Git-visible
existing change as its baseline, and uses temporary child worktrees only for safe parallel
implementation. If they name files/directories such as
"use /a/b/design.md to implement in /g/repo", pass them as `contextPaths:["/a/b/design.md"]`.
Those paths are live read-only references; never paste their contents into a prompt.
For `feature_campaign`, a single `backend` launcher value maps to design, plan, code, and
review; the workflow pauses at phase-aware WAIT checkpoints, keeps its verified branch local by
default, and publishes remotely only when the user explicitly selects `createPr:true`.
...
- code_parallel, issue_to_pr, and address_pr (code_parallel engine) inspect the target checkout
  before planning. Use OpenSpec only when that checkout already contains an `openspec/` folder;
...
For a pending pr_review checkpoint, use get_approval when the user wants to inspect the current
draft or prior private answers. The investigate action requires a specific question, consumes one
of at most five private passes, resumes the read-only reviewer, and never posts to GitHub. After the
refreshed checkpoint appears, the user may investigate again, approve, request changes, stop, or
leave it for later. Keep investigation questions separate from request-changes feedback.
