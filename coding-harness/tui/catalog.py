...
def _r_design_docs(o: dict) -> ResultCard:
    ...
    rows = [
        ...
        ("tokens", str(o.get("tokenUsed", 0))),
        ("cost", f"${float(o.get('costUsd') or 0):.2f}"),
        ("prompt", _prompt_source(o.get("designPromptTemplate"))),
    ]
...
CATALOG: dict[str, WorkflowSpec] = {
    ...
    "issue_to_pr": WorkflowSpec(
        ...
        fields=(
            ...
            Field("backend", "Backend override", "enum", "", choices=("", *_BACKENDS),
                  maps_to=("planAgent", "codeAgent"), help="blank = selected model profile"),
            ...
            Field("designAgent", "Design backend override", "enum", "", choices=("", *_BACKENDS), advanced=True),
            _MODEL_PROFILE,
...
LAUNCHABLE = ["local_review", "pr_review", "issue_to_pr", "address_pr", "pr_review_sweep", "pr_address_sweep", "issue_resolution_sweep", "openspec_development", "feature_campaign", "code_parallel", "design_docs"]
