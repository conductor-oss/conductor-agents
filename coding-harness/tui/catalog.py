...
def _caps(max_turns: int, budget: float, timeout: int) -> tuple[Field, ...]:
    return (
        ...
        Field("timeoutS", "Timeout (s)", "int", timeout, advanced=True,
              help="wall-clock cap per agent"),
    )
...
CATALOG: dict[str, WorkflowSpec] = {
    ...
    "issue_to_pr": WorkflowSpec(
        ...
        fields=(
            ...
            Field("designHumanApproval", "Human design review", "bool", True,
                  help="pause after every design pass; off = read-only LLM judge"),
            ...
            Field("designReviewAgent", "Design judge backend", "enum", "claude",
                  choices=_BACKENDS, advanced=True),
            Field("designReviewModel", "Design judge model", "text", "", advanced=True),
            Field("designReviewMaxTurns", "Design judge max turns", "int", 5, advanced=True),
            Field("designReviewMaxBudgetUsd", "Design judge budget $", "float", 0.75, advanced=True),
    ...
    "design_docs": WorkflowSpec(
        ...
        fields=(
            ...
            Field("designMaxTurns", "Max turns", "int", 40, advanced=True),
            Field("designMaxBudgetUsd", "Max budget $", "float", 1.5, advanced=True),
            Field("humanApproval", "Human design review", "bool", True,
                  help="pause after every pass; off = read-only LLM judge"),
            Field("designMaxIterations", "Design iterations", "int", 5, advanced=True),
            Field("designReviewAgent", "Judge backend", "enum", "claude",
                  choices=_BACKENDS, advanced=True),
            Field("designReviewModel", "Judge model", "text", "", advanced=True),
            Field("designReviewMaxTurns", "Judge max turns", "int", 5, advanced=True),
            Field("designReviewMaxBudgetUsd", "Judge budget $", "float", 0.75, advanced=True),
            Field("timeoutS", "Timeout (s)", "int", 600, advanced=True),
        ),
        target=_t_local,
...
DASHBOARD_TYPES = ["pr_review", "issue_to_pr", "address_pr", "code_parallel", "github_demo"]
