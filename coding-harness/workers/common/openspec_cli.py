...
TASKS_RULE = (
    ...
    "The `Test:` command MUST be exactly one program invocation, executed "
    "directly without a shell: no pipes (`|`), chaining (`&&`, `||`, `;`), "
    "command substitution (`$(...)` or backticks), or shell builtins like "
    "`test`/`[`. The coding agent implementing this task group will only be "
    "permitted to run that literal command (plus a small default set of "
    "read-only/git commands) — it will be denied permission to run anything "
    "else, so the command must be something that agent can actually execute "
    "standalone. Prefer a single call to `pytest`, `npm`, `npx`, `go`, "
    "`cargo`, `node`, or `python3` (use `python3`, not `python` — this "
    "environment only guarantees `python3` on PATH). If verifying a task "
    "group genuinely needs multiple steps, write one test file/command that "
    "covers all of them (e.g. a `pytest` file asserting every case) rather "
    "than chaining shell commands together.\n"
