...
- A verification worker must never silently drop a real, narrowly-scoped candidate command because a heuristic (a heavy-marker substring match, an authored-test shape check) misclassified it. When `command_discovery_blocked` occurs, first rule out a known false-block class before treating it as a genuine "cannot verify" result.
- A heavy-suite marker (`acceptance`, `e2e`, `integration`, …) must never be scanned against the VALUE of a single-target test-filter flag (`--tests`, `-Dtest=`, `-k`, `--filter`, `-R`, `TEST=`/`SPEC=`). That value names one already-selected test, picked mechanically from the actual changed file, never a task or script a human or agent chose. The exemption must be shape/position-based (which argv slot the token occupies), never string-based (which characters the token contains) — a heavy task literally named `e2eTest` or a script named `run-integration-suite.sh` still occupies a different argv slot and must still be blocked.
- An agent-authored-test shape check must never attribute a path to the agent's own change unless the path's git status genuinely differs from an immediately-prior baseline snapshot. Re-scanning the whole worktree's current `git status` and demanding "exactly one path" against that full scan blames the agent for anything else dirty in the workspace, including generated build artifacts (`.gradle`, `build`, `target`, …) an agent with no Bash tool could never have produced.
- Every workflow that invokes the `test_cycle` sub-workflow must forward `allowAgentTestPlan` and `allowAgentAuthoredTests` from its own inputs into that call. A caller's fallback preference must never silently collapse to `test_cycle`'s own `false`/`false` default because a wiring layer omitted the flag.
- `scripts/audit_harness_invariants.py`'s `HEAVY_MARKER_FALSE_BLOCK`, `AUTHORED_TEST_SHAPE_ATTRIBUTION`, and `TEST_CYCLE_FALLBACK_FORWARDING` checks enforce the structural half of this policy. They prove the known workflow graph and known heuristics are closed; they do not prove every repository is verifiable. A repository with no derivable, discoverable, or authorable test may still end `command_discovery_blocked` — that is a distinct, honestly-reported outcome, and must never be silently converted into a passing result.
