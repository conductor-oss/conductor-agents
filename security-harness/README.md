...
For real engagements, use `--manifest <file>` instead. A manifest specifies: approvers, in-scope hosts, testing window, capability ceiling, rate/volume budgets, and forbidden operations. It is validated at startup and **fails closed**.

**Capability levels gate every action:**

| Level | What it permits |
|---|---|
| 0 | Passive recon only |
| 1 | Read-only active probes |
| 2 | State-changing tests, product-feature exploitation |
| 3 | Sensitive proof collection |
| 4 | Destructive (prohibited by default; the harness can never self-escalate) |

An independent safety governor halts the campaign on window expiry, a `--kill-switch` file, or a policy breach. A tamper-evident audit log records every action.
